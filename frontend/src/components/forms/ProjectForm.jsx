...
function ProjectForm({ changed,entries, addArrayItem, updateArrayItem, removeArrayItem }) {
  ...
  const createNewEntry = () => {
    ...
    addArrayItem("projects", projectData);
  ...
  changed(true);
    setProjectData((prev) => {
      ...
      if (editIndex !== -1) updateArrayItem("projects", editIndex, updated);
      return updated;
    });
  ...
  // Delete project
  const handleDelete = () => {
    if (editIndex !== -1) removeArrayItem("projects", editIndex);
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {entry.startDate ? new Date(entry.startDate).getFullYear() : ""} - {entry.endDate ? new Date(entry.endDate).getFullYear() : ""}
                </p>
              </div>
              <button
